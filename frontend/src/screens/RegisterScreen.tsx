...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        const data = (await response.json()) as RegisteredUser;

        setRegisteredUser(data);
        setProfileName(data.name);
        setProfileEmail(data.email);

        setSuccessMessage(`Account created successfully! User ID: ${data.id}`);
        setName("");
        setEmail("");
        setPassword("");
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const updatedUser = data as RegisteredUser;
        setRegisteredUser(updatedUser);
        setProfileName(updatedUser.name);
        setProfileEmail(updatedUser.email);
        setSuccessMessage(
          `Profile updated: ${updatedUser.name} (${updatedUser.email})`
        );
      } else {
        setErrorMessage(
      ...
      >
        ...
        >
          ...
          <View style={s.card}>
            ...
            <Text style={s.profileTitle}>Login</Text>
            <Text style={s.profileSubtitle}>
              Log in to access and update your profile.
            </Text>
            ...
            <TouchableOpacity
              style={[s.button, isLoggingIn ? s.buttonDisabled : s.buttonActive]}
              onPress={handleLogin}
            ...
            {registeredUser && (
              <View style={s.profileCard}>
                <Text style={s.profileTitle}>Demo: Update Profile</Text>
                <Text style={s.profileSubtitle}>
                  Editing user #{registeredUser.id}
                </Text>
                ...
                <TouchableOpacity
                  style={[s.button, isUpdatingProfile ? s.buttonDisabled : s.buttonActive]}
